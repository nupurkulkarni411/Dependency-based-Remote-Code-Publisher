...
private:
	bool recvFile(const std::string& filename, size_t fileSize, Socket& socket,const std::string& fromAddr, const std::string& filepath);
	Async::BlockingQueue<std::string>& clientRecvQ_;
	void openRequestedWebPage(const std::string& toAddr,const std::string& filename, const std::string& filepath);
};
	...
	while (true)
	{
		...
		if (httpMsg.command() == "publish")
		{
			...
			for (int i = 0; i < 1; i++)
			{
				...
				{
					std::cout << ("\n\n  Upload error : did not receive file\n\n");
					break;
				}
				recvFile(fTransHttpMsg.name(), fTransHttpMsg.getContentLength(), socket_,fTransHttpMsg.toAddress(),httpMsg.content());
			}
		}
		clientRecvQ_.enQ(httpMsg.buildMessage());
		if (httpMsg.command() == "open_home")
			openRequestedWebPage(httpMsg.toAddress(),httpMsg.name(),httpMsg.content());
		if (httpMsg.command() == "file_list") {
			std::cout << "\n";
			std::vector<std::string> body = httpMsg.getBody();
			for (auto i : body)
				std::cout <<"   " << i<<"\n";
		}
...
bool ServerHandler::recvFile(const std::string& filename, size_t fileSize, Socket& socket, const std::string& toAddr, const std::string& filepath)
{
	std::string fqname;
	fqname = filepath + "//" + filename;
...
void ServerHandler::openRequestedWebPage(const std::string& toAddr,const std::string& filename, const std::string& filepath)
{
	std::string homeLink;
	homeLink = FileSystem::Path::getFullFileSpec( filepath + "//"+filename + ".htm");
...
public:
	...
	void manualExecution(std::string testsuit);
private:
	bool startSender();
	bool startReceiver();
	bool sendProc();
...
//----Dispaying manual execution -----//

void MockChannel::manualExecution(std::string TestSuit)
{
	std::cout << "\n\n-----------------------------Requirement 9-------------------------------------\n\n";
	std::cout << "   Manual demostration of completion of requirements of project 4: \n";
	std::cout << "   1. Uploading all files in TestSuit folder to repository demo category";
	HTTPMessage msg;
	msg.command() = "upload";
	...
	std::string testsuitpath = FileSystem::Path::getFullFileSpec(TestSuit);
	std::vector<std::string> files = FileSystem::Directory::getFiles(testsuitpath);
	for (auto i:files)
		dep.push_back(FileSystem::Path::getFullFileSpec(testsuitpath+"//"+ i));
	msg.setBody(dep);
	msg.setContentLength(dep.size());
	postMessage(msg.buildMessage()); Sleep(3000);
	std::cout << "\n   Files Uploaded.\n";
	std::cout << "\n   2. Demonstrating Get repository contents:\n\n ";
	...
	msg.setBody(vec);
	postMessage(msg.buildMessage()); Sleep(3000);
	std::cout << "\n   3. Demonstrating Delete stored files, as requested by a local client:\n";
	std::cout << "      Deleting DepAnal.cpp";
	msg.command() = "delete_file";
	msg.name() = "DepAnal.cpp";
	msg.content() = "no_content";
	msg.setBody(vec);
	postMessage(msg.buildMessage());
	std::cout << "\n   File Deleted.\n"; Sleep(3000);
	msg.command() = "get_file_list"; msg.name() = "no_name"; msg.content() = "no_content";
	msg.setBody(vec); postMessage(msg.buildMessage()); Sleep(200);
	std::cout << "\n   4. Please use GUI for checking publishing of files.\n\n ";
	std::cout << "   Please uncomment the commented code according to the relative path of current directory and ClientDump folder.";
	std::cout << "\n   for automating publish functionality. Please read ReadMe for further details.";
