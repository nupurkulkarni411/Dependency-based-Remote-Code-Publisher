...
{  
  ...
  public:
	...
	TypeTable& typeTable() { return ttable; }



	Graph<std::string, std::string>& dependencyGraph() { return graph; }
  ...
  public:
	  bool doTest(const Scanner::ITokCollection* pTc) override
	  {
		  GrammarHelper::showParseDemo("Test typedef definition: ", *pTc);
		  Repository* pRepo = Repository::getInstance();
		  ...
		  //checkIsPublic(isPublic, pTc, pRepo, parentType);
  ...
  ///////////////////////////////////////////////////////////////
  // action to add typedef info to scope stack top

  class HandleTypedefDefinition : public IAction
  {
	  Repository* p_Repos;

  public:
	  HandleTypedefDefinition(Repository* pRepos)
	  {
		  p_Repos = pRepos;
	  }
	  void doAction(const Scanner::ITokCollection* pTc) override
	  {
		  GrammarHelper::showParseDemo("Handle typedef definition: ", *pTc);
		  ...
		  /*else if (pTc->find("using") < pTc->length() && (pTc->find("namespace") < pTc->length()))
		  {
			  std::string name = (*pTc)[pTc->find("=") - 1];
			  top->type_ = "using";
			  top->name_ = name;
		  ...
		  GrammarHelper::showParse("typedef def", *pTc);
	  }
  };
  ...
  public:
    ...
    void doAction(const Scanner::ITokCollection* pTc) override {
      ...
      std::string name = (*pTc)[nameIndex];
      if (nameIndex > 0 && (*pTc)[nameIndex - 1] == "~"){ //is function destructor?
        --nameIndex;
        name = "~" + name;
      }
      top->type_ = "function";
      ...
      size_t operIndex = pTc->find("operator"); //is function operator?
      ...
      if (nameIndex > 1 && (*pTc)[nameIndex - 1] == "::") // is function a member of a class or struct?
      {//----< start find class name >--------------------
        std::string className = (*pTc)[nameIndex - 2];
        if (className == ">"){// is class a template?
        ...
        return;}
      std::string packageName = p_Repos->package(); // is this a lambda?
