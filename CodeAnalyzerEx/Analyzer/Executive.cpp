...
* Package Operations:
* -------------------
* This class demonstrates Code Analyzer.
* - Complexity and Metric Analysis.
* - Dependency Analysis.
* - Type Analysis.
* - Strong Components.
* - Saves data to NoSqlDatabase.
*
* Required Files:
* ---------------
*   - Executive.h, Parser.h, FileSystem.h, FileMgr.h, ActionsAndRules.h, ConfigureParser.h,
*   - AbstrSynTree.h, Logger.h, Utilities.h, TypeAnal.h, DepAnal.h
*
* Maintenance History:
* --------------------
* ver 1.0 : 27 Feb 2017
* - demonstrates the requirements.
* ver 1.1 - 2 March 2017
* - incorporated type analysis, dependency analysis and strong components
*/
...
void CodeAnalysisExecutive::processCppHeaderFiles(bool showProc)
{
...
void CodeAnalysisExecutive::processCppImplemFiles(bool showProc)
{
...
//showing requirement 3
void CodeAnalysis::CodeAnalysisExecutive::requirement2()
{
	std::cout << "   -----------------------------------------------------------------------------------------";
	std::cout << "\n\t\t\t\t  Requirement 2 \n";
	std::cout << "   -----------------------------------------------------------------------------------------";
	std::cout << "\n\n    Using C++ standard library's streams for all I/O and new and delete for all heap-based memory management.\n";
}

//showing requirement 3
void CodeAnalysis::CodeAnalysisExecutive::requirement3(int argc, char* argv[])
{
	std::cout << "\n   -----------------------------------------------------------------------------------------";
	std::cout << "\n\t\t\t\t  Requirement 3 \n";
	std::cout << "   -------------------------------------------------------------------------------------------";
...
//showing requirement 4
void CodeAnalysis::CodeAnalysisExecutive::requirement4()
{
	std::cout << "\n\n   -----------------------------------------------------------------------------------------";
	std::cout << "\n\t\t\t\t  Requirement 4 \n";
	std::cout << "   -----------------------------------------------------------------------------------------";
	std::cout << "\n     Facility provided to expand or collapse class bodies, methods, and global functions using JavaScript and CSS properties.\n\n";
}

//showing requirements 5,6,7
void CodeAnalysis::CodeAnalysisExecutive::requirement5(int argc, char* argv[])
{
	std::cout << "\n   -----------------------------------------------------------------------------------------";
	std::cout << "\n\t\t\t\t  Requirement 5 ,6 and 7\n";
	std::cout << "   -----------------------------------------------------------------------------------------";
	std::cout << "\n     Specification of the CSS file:\n\n";
	std::string file = FileSystem::Path::getFullFileSpec(argv[argc-2]) + "\\" + "CSSFile.css";
	...
	std::cout << "\n     Specification of the JavaScript file:\n\n";
	file = FileSystem::Path::getFullFileSpec(argv[argc - 2]) + "\\" + "JSFile.js";
	std::ifstream in1(file);
	do {
		std::getline(in1, line);
		cout << " " << line << "\n";
	} while (in1.good());
	std::cout << "\n     Showing one *.htm file to show web page's <head> section links to ";
	std::cout << "the style sheet and JavaScript file:\n\n";
	std::cout << "       <link rel = stylesheet href = CSSFile.css>\n";
	std::cout << "       <script src = JSFile.js></script>\n";
	std::cout << "\n     My code publisher package contains ConvertToHtml.h which depends on ConvertToHtml.cpp ";
	std::cout << "\n     Showing Html code for ConvertToHtml.h.htm:\n\n";
	std::cout << "       <h3>ConvertToHtml.h< / h3>\n";
	std::cout << "       <hr / >\n";
	std::cout << "       <div class = indent><h4>Dependencies:< / h4><p><a href = ConvertToHtml.cpp.htm> ConvertToHtml.cpp< / a>< / p>< / div>\n";
	std::cout << "       <hr / >\n";
}

//showing requirement 8
void CodeAnalysis::CodeAnalysisExecutive::requirement8(int argc, char * argv[])
{
	std::cout << "\n   -----------------------------------------------------------------------------------------";
	std::cout << "\n\t\t\t\t  Requirement 8  \n";
	std::cout << "   -------------------------------------------------------------------------------------------";
	std::cout << "\n     Showing parts of command line:";
	std::cout << "\n     Path of files to be analyzed: \"" << FileSystem::Path::getFullFileSpec(argv[1]);
	std::cout << "\n     File patterns:";
	for (int i = 2; i <= argc - 3; ++i)
		std::cout << argv[i] << ", ";
	std::cout << "\n     Output folder path in which all .htm files will be stored:\n";
	std::cout << "      " <<FileSystem::Path::getFullFileSpec(argv[argc - 2]);
	std::cout << "\n     File name cited in command line which will be displayed in default browser :" << argv[argc - 1];
}

void CodeAnalysisExecutive::generateHTML(int argc, char* argv[])
{
	TypeAnal ta;
	ta.doTypeAnal();   //type analysis
	DepAnal depAnal;
	depAnal.ProcessCommandLine(argc, argv);
	depAnal.getSourceFiles();
	depAnal.processSourceCode();    // dependancy analysis 
	std::string DestFolderPath = FileSystem::Path::getFullFileSpec(argv[argc-2]);	
	FileSystem::Directory::create(DestFolderPath);

	for (auto file : cppHeaderFiles())
	{
		ConvertToHtml cth;
		cth.SetFileName(file);
		cth.SetDestDirName(DestFolderPath);
		cth.InsertPrologue();
...
//function for deleting directory if already exists
void CodeAnalysisExecutive::DelDirectory(std::string & directory)
{
	std::vector<std::string> files = FileSystem::Directory::getFiles(directory);
	// remove all the files inside the directory
	for (std::string filePath : files)
		FileSystem::File::remove(directory + "\\" + filePath);

	// get directories list inside of the directory you want to delete
	std::vector<std::string> directories = FileSystem::Directory::getDirectories(directory);
	for (std::string dirPath : directories) {
		// YOU MUST SKIP THESE OR YOU WILL ENTER IN INFINITE LOOP
		if (dirPath == "." || dirPath == "..") continue;
		// call the function recursively on child directories
		DelDirectory(directory + "\\" + dirPath);
	}
	// at this moment, the directory is empty and all 
	// its sub-directories are deleted, so just delete it now
	FileSystem::Directory::remove(directory);
}

void CodeAnalysis::CodeAnalysisExecutive::setPath(std::string & path)
{
	path_ = path;
}

void CodeAnalysis::CodeAnalysisExecutive::setPattern(std::vector<std::string>& patterns)
{
	patterns_ = patterns;
}

void CodeAnalysis::CodeAnalysisExecutive::setOptions()
{
	options_ = {};
}

int CodeAnalysis::CodeAnalysisExecutive::AnalysisPreprocessing()
{
...
int main(int argc, char* argv[])
{
	...
	try {
		...
		exec.stopLogger();
		exec.requirement2();
		exec.requirement3(argc, argv);
		exec.requirement4();
		exec.generateHTML(argc, argv);
		ConvertToHtml cth2;
		std::string DestFolderPath = FileSystem::Path::getFullFileSpec(argv[argc - 2]);
		cth2.SetDestDirName(DestFolderPath);
		std::vector<std::string> files = FileSystem::Directory::getFiles(DestFolderPath, "*.htm");
		cth2.CreateHomePage(files);
		exec.requirement5(argc, argv);
		exec.requirement8(argc, argv);
		cth2.OpenCommandLineFile(argc, argv);
