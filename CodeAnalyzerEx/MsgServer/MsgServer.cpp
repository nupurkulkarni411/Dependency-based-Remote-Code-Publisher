...
*   FileSystem.h, FileSystem.cpp,TypeAnal.h,DepAnal.h,ConvertToHtml.h
*
...
{
public:
	ClientHandler(BlockingQueue<std::string>* pBQ, std::string repopath) : serverRecvQ(*pBQ),repoPath(repopath) {}
	...
	while (true)
	{
		...
		if (httpMsg.command() == "upload")
		{
			for (int i = 0; i < std::stoi(httpMsg.content()); i++)
			{
				...
				std::string tempDir = repoPath + "//" + httpMsg.name();
...
{
	std::string fqname = repoPath + "//" + folder + "/" + filename;
...
public:
	...
	RepositoryServer(std::string addr, size_t port,std::string repoPath);
	bool startListener();
...
private:
	...
	void doPublishWithDep(std::vector<std::string> visited, std::string fileName, std::string toAddr, std::string filepath);
	std::vector<std::string> getDependencies(std::string pkgName, std::string filename);
	bool serverProc();
	bool sendProc();
	bool sendFile(const std::string& fileName, SocketConnecter& sc, std::string fromAddr, std::string toAddr,std::string filepath);
...
{
	std::string htmPath = FileSystem::Path::getFullFileSpec(repoPath +"//" + pkgName + "/" + filename + ".htm");
...
RepositoryServer::RepositoryServer(std::string addr, size_t port,std::string repoPath) : ss(), address_(addr), port_(port), sl(port), ch(&recvQ_,repoPath),repoPath(repoPath) {}
...
bool RepositoryServer::sendFile(const std::string& fileName, SocketConnecter& sc, std::string fromAddr, std::string toAddr, std::string filepath)
{
...
{
	std::string dfpath = FileSystem::Path::getFullFileSpec(repoPath);
...
bool RepositoryServer::sendProc()
{
	while (!closeSender_)
	{
		...
		if (hMsg.command() == "publish")
		{
			...
			std::string filePath;
			std::string dirPath;
			std::vector<std::string> dirs = FileSystem::Directory::getDirectories(repoPath);
			for (auto dir : dirs){
				if (dir != "." && dir != "..")
				{
					std::vector<std::string> file_list = FileSystem::Directory::getFiles(repoPath+"//" + dir);
					for (auto f : file_list) {
						if (f == hMsg.name()){
							filePath = repoPath + "//" + dir + "/" + f;
							dirPath = repoPath + "//" + dir;
						}
					}
				}
			}
			...
			sendFile(filePath, connections_.find(toAddr)->second, hMsg.fromAddress(), hMsg.toAddress(),hMsg.content());
		}
		else
			connections_.find(toAddr)->second.sendString(hMsg.buildMessage());
...
void RepositoryServer::doPublishWithDep(std::vector<std::string> visited_, std::string fileName, std::string toAddr,std::string filepath)
{
	...
	httpMsg.name() = "CSSFile.css"; httpMsg.content()=filepath;// sending CSS file 
...
bool RepositoryServer::serverProc()
{
	...
	while (!closeServer_) {
		...
		if (httpMsg.command() == "upload") {
			...
			std::vector<std::string> dirs = FileSystem::Directory::getDirectories(repoPath);std::vector<std::string> packages;
			for (auto dir : dirs) {
				if (dir != "." && dir != "..") {
					packages.push_back("<> " + dir);
					std::vector<std::string> file_list = FileSystem::Directory::getFiles(repoPath+"/" + dir);
			...
			for (auto dir : dirs) {
				if (dir != "." && dir != "..") {
					...
					for (auto f : file_list) {
						if (f == httpMsg.name())
							FileSystem::File::remove(FileSystem::Path::getFullFileSpec(repoPath+"//" + dir + "//" + f));
					}}}httpMsg.command() = "file_deleted"; httpMsg.content() = "delete";}
		else if (httpMsg.command() == "open_web_page") {
			std::cout << ("\n  server processing get_package_list request for client : " + httpMsg.fromAddress() + "\n");
			std::vector<std::string> visited{}; doPublishWithDep(visited, httpMsg.name().substr(0,httpMsg.name().find_last_of(".")), httpMsg.fromAddress(), httpMsg.content());
			continue;}
			...
			std::vector<std::string> visited{}; doPublishWithDep(visited,httpMsg.name(), httpMsg.fromAddress(),httpMsg.content());
			continue;
		}
...
int main(int argc, char* argv[])
{
	...
	std::cout << FileSystem::Directory::getCurrentDirectory();
	std::cout<<"\n  Remote Repository Server started on port : ";
	try
	{
		std::cout << std::stoi(argv[1]);
		RepositoryServer server("localhost", std::stoi(argv[1]),argv[2]);
		server.startSender();
